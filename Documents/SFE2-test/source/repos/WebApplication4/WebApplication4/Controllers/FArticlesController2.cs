    ...
    public class FArticlesController2 : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] FamilleArticle familleArticle)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new MySqlCommand("INSERT INTO FamilleArticle (Nom, Description, Prix, Image, DateCreation, Categorie) VALUES (@Nom, @Description, @Prix, @Image, @DateCreation, @Categorie)", conn))
                    {
        ...
        public async Task<IActionResult> Edit(int id, [FromBody] FamilleArticle familleArticle)
        {
            ...
            {
                conn.Open();
                using (var cmd = new MySqlCommand("UPDATE FamilleArticle SET Nom = @Nom, Description = @Description, Prix = @Prix, Image = @Image, DateCreation = @DateCreation, Categorie = @Categorie WHERE FamilleArticleId = @Id", conn))
                {
                    cmd.Parameters.AddWithValue("@Nom", familleArticle.Nom);
                    cmd.Parameters.AddWithValue("@Description", familleArticle.Description);
                    cmd.Parameters.AddWithValue("@Prix", familleArticle.Prix);
            ...
            return Ok(new { message = "FamilleArticle updated successfully." });
        }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            return Ok(new { message = "FamilleArticle deleted successfully." });
        }
    }
}